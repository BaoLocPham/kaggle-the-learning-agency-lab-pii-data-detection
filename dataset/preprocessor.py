...
class Preprocessor:
    ...
    def ner_overlap_count(self, row, mode: str):
        ...
        if "spacy" in str(model):
            prompt_ner = set([(token.text, token.label_)
                             for token in prompt.ents])
            summary_ner = set([(token.text, token.label_)
                              for token in summary.ents])
        elif "stanza" in str(model):
            prompt_ner = set([(token.text, token.type)
                             for token in prompt.ents])
            summary_ner = set([(token.text, token.type)
                              for token in summary.ents])
        else:
...
#         from IPython.core.debugger import Pdb; Pdb().set_trace()
        ...
        input_df['length_ratio'] = input_df['summary_length'] / \
            input_df['prompt_length']
