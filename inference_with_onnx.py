...
def post_process_predicts(processed_predictions, ds, id2label):
    ...
    for p, token_map, offsets, tokens, doc, full_text in zip(
        ...
        for token_pred, (start_idx, end_idx) in zip(p, offsets):
            ...
            if label_pred in ("O", "B-EMAIL", "B-PHONE_NUM",
...
def infer(cfg):
    ...
    ds = ds.map(
        ...
        fn_kwargs={
            "tokenizer": tokenizer,
            "config": config.inference_with_onnx},
        batched=False,
        num_proc=2,
        desc="Tokenizing",
    )
    ...
    for sample_index in range(len(predictions_softmax_logits[0])):
        ...
        for model_index in range(len(predictions_softmax_logits)):
            weighted_prediction = predictions_softmax_logits[model_index][sample_index] * (
